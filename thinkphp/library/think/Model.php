...
abstract class Model implements \JsonSerializable, \ArrayAccess
{
	...
	public static function getBaseData($file = ' * ',$data = array(),$page = '',$listRow = '') {
		$prefix			= Config::get('database')['prefix'];
		$database_table	= $prefix.self::$model_table;
		...
		if(!empty($page) && !empty($listRow)) {
			$limit		= " LIMIT ".(($page - 1) * $listRow).",$listRow ";
		}
		$sql			= "SELECT $file FROM $database_table WHERE $where $limit";
		$ret			= \think\Db::query($sql);
		return $ret;
	}
